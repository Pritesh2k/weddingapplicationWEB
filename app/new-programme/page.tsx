...
import { EMPTY, TOTAL_STEPS } from '@/lib/NewProgramme/constants'
import { SUGGESTED_EVENTS }   from '@/lib/NewProgramme/constants'
...
export default function NewProgramme() {
  ...
  const next = () => {
    ...
    const final = { id, ...data, createdAt: new Date().toISOString() }
    try {
  ...
  const addEvent    = () => patch({ subEvents: [...data.subEvents, { id: `evt_${Date.now()}`, name: '', date: data.dateFrom || '', startTime: '', endTime: '' }] })
  const removeEvent = (id: string) => patch({ subEvents: data.subEvents.filter((e) => e.id !== id) })
  ...
  const toggleCulture  = (c: CultureModule) => {
    const next = data.cultures.includes(c) ? data.cultures.filter((x) => x !== c) : [...data.cultures, c]
        ...
        <div className="w-full max-w-xl">
          ...
          <div style={cardStyle}>
            ...
            >
              ...
              <StepNavigation step={data.step} onBack={back} onNext={next} />
            </div>
          </div>
